...
Status: increments 1Aâ€“1C are implemented; 1D (integration polish) is partial. The
...
| `:app:connectedDebugAndroidTest` (AOSP API 35 emulator) | 20 of 20 passed on 1080Ã—1920 @ 420 dpi; on 1600Ã—1000 @ 160 dpi (rail + detail pane); and on 1080Ã—1920 with all animation scales set to 0 (reduced motion) |
...
Instrumented tests (run on the emulator) cover: v1â†’v2 migration keeping the theme; close/reopen persistence of
...
- Memory while paging through the 3.16 GB archive stayed flat: Java heap about 16â€“20 MB,
  native heap about 54â€“57 MB, total PSS 209â€“216 MB after 1, 61 and 121 pages.
- The review dialog appeared within about 5â€“9 s of choosing a file, including picker
  dismissal and UI-automation polling overhead.
- Settings â†’ Storage reported no unused private copies (reference imports only).
...
**Emulator validation (AOSP API 35, 1080Ã—1920 @ 420 dpi).** `connectedDebugAndroidTest`:
...
against the pre-remediation reader code. Not repeated after the fixes: the 1600Ã—1000,
...
- A preparation that completes at the moment its import is cancelled is not handed back
  ...
  under Settings â†’ Storage as an unused copy.
...
Codex's focused re-review passed the R1â€“R5 remediation (`PHASE1_CODEX_REVIEW.md`, "Final
Verification"). This pass addresses the remaining acceptance gates: device, accessibility,
input, performance and motion validation. It does not reopen R1â€“R5.
...
| Check | Result |
| --- | --- |
| `gradlew :app:testDebugUnitTest --tests com.d4guilar.shelfos.ReviewRegressionTest --init-script .tools/codex-review.init.gradle --rerun` | PASS: 3 run, 0 failed. R1â€“R5 remain resolved |
...
1. Add file â†’ real document picker â†’ Downloads â†’ the fixture. ShelfOS's own semantic Escape
...
2. Review dialog showed the correct format/size (EPUB Â· 5 KB) and the title read from
...
5. The Read button stayed above the fold on the details screen, and the adaptive rail
   ...
   1920Ã—1080 landscape window, without a device-model check.
...
| --- | --- | --- |
| Cold start | `adb shell am start -W -n com.d4guilar.shelfos/.MainActivity` | 714â€“724 ms `TotalTime`, twice |
| Frame timing while paging the EPUB reader (5 page turns) | `dumpsys gfxinfo com.d4guilar.shelfos` | 50th/90th/95th/99th percentile 10/44/150/250 ms combined across two activity windows; the reported "janky (legacy)" rate ranged 3â€“21% depending on window. Not compared against a target: none is documented |
...
- Rotation/recreation does not produce a broken transition: confirmed by the corrected
  `EpubRecreationTest` on physical hardware â€” after a real rotation, controls stay hidden
...
- **The documented cover-expansion reader transition (`docs/design/CLASSIC_UI.md` Â§12) is
...
landscape-locked hardware was fixed without touching R1â€“R5 or production code.
...
None of these are R1â€“R5 findings. See the [physical device acceptance
...
## Phase 1 acceptance closure â€” continuation (2026-09-24)

Continues the closure pass above. New automated coverage was found already in the working
tree at the start of this continuation â€” `LibraryPersistenceTest` gained non-seekable
...
- **Non-seekable provider.** A debug-only `ContentProvider` returns a pipe descriptor
  (`ParcelFileDescriptor.createReliablePipe()`), not a file. `nonSeekableProviderCopiesâ€¦`
...
- **Room rollback.** A SQLite trigger injected only for this test aborts the delete inside
  ...
  no production change â€” `remove()` was already `@Transaction`.
...
the same run as several preceding tests â€” see Compatibility below for that emulator-specific
finding.
...
- App launches (`am start -W`: COLD, succeeds) and the full `NavigationSmokeTest` class â€”
  ...
  zoom/fit + global-Appearance test, and the new scroll-restoration/rapid-switch test â€”
  **passes 12 of 12.**
- `LibraryPersistenceTest`'s 10 cases pass individually and in small groups. Run together
  ...
  - The crash is inside the platform's bundled `libpdfium.so`, not ShelfOS code, and other
    tests exercising the same `PublicationFiles` â†’ `PdfRenderer` path (`pdfAndArchiveRenderâ€¦`,
    `copyImportDetectsâ€¦`) pass repeatedly on this same emulator.
...
- The system image boots and **ShelfOS itself launches and renders correctly**
  ...
  as expected).
- Non-UI instrumented tests â€” `LibraryPersistenceTest`, `RoomPersistenceTest`,
  `ReaderStateTest` (13 tests: Room, import/file logic, locator/preference serialization) â€”
  **pass 13 of 13.**
- Automated Compose UI instrumented tests (everything that needs Espresso's `onIdle`, i.e.
  ...
  `InputManager.getInstance()` accessor that this platform version has removed â€” a test-
...
control has a visible text label and a â‰¥48dp touch target).
...
TalkBack, on the RP5 or on the `shelfos-api37` emulator (already has TalkBack installed) â€”
...
hardware remain **NOT VERIFIED** â€” see the physical checklist.

### PERFORMANCE

A representative synthetic load, generated for this pass and not committed to Git: a 219 MB,
160-page CBZ (1200Ã—1800 JPEG pages), run on the physical Retroid Pocket 5.

| Measurement | Method | Result |
| --- | --- | --- |
| Memory while paging (page 1 â†’ page 81 of 160) | `Debug.MemoryInfo().totalPss` before/after, logged by the test | 335,750 KB â†’ 346,326 KB PSS (roughly flat over 80 page turns); Java heap 9.6 MB |
...
| Cold start (trivial fixture, from the previous pass) | `adb shell am start -W` | 714â€“724 ms, RP5; 1794 ms, API 37 emulator (first cold start after install, includes APK verification) |
...
then switches Library â†’ Manga reader â†’ Library â†’ Books reader â†’ Library twice more in
...
transition (`docs/design/CLASSIC_UI.md` Â§12) remains unimplemented. See the previous
...
None of these are R1â€“R5 findings.

### Physical controller â€” owner-verified (2026-09-24)
...
with no issues observed during normal Library and Reader interaction â€” physical D-pad/
...
## Phase 1 acceptance closure â€” final items (2026-09-24)
...
a manual checklist (below). R1â€“R5 and everything already resolved in the two earlier
closure sections were not reopened.

### Cover-expansion reader transition (CLASSIC_UI.md Â§12)

**Implemented â€” the smallest faithful version.** `docs/design/CLASSIC_UI.md` Â§12: "selected
...
The real navigation â€” the same `NavHost` route change or `EpubActivity` launch as before â€”
...
directly (API 24â€“25, since `ValueAnimator.areAnimatorsEnabled()` requires API 26 and minSdk
is 24). When true, `openReader` skips the overlay and navigates immediately â€” the
...
**Regression coverage:**
- JVM: `CoverExpandTransitionTest` (3 tests) â€” chrome is fully visible before the transition
...
- Instrumented: `NavigationSmokeTest.readerEntryTransitionIsSkippedUnderReducedMotion`
  (new) â€” forces `animator_duration_scale=0` via a shell command, confirms Read still opens
  the reader. All of `NavigationSmokeTest`'s existing Detailsâ†’Read cases
...
- Physical device (Retroid Pocket 5): full instrumented suite, three separate runs across
  this pass â€” 28/28 with a partial suite once, then 29/29, 29/29 (clean rebuild), and 29/29
  ...
  not a regression â€” the failure pattern (test-infrastructure registration errors,
...
| Property | Value |
| --- | --- |
| Generated size | 2,997,685,907 bytes (â‰ˆ2.99 GB) |
| Page count | 947 (1600Ã—2400 JPEG pages, stored uncompressed-in-ZIP so archive size matches page bytes exactly) |
| Device / API | Retroid Pocket 5, API 33 |
| Open time | 534â€“535 ms (two runs) from tapping Read to the first page rendering |
| Pages visited | 1 through 180, by the same sequential Next control the UI exposes (no private API) |
| Memory (`Debug.MemoryInfo`, in-process) | PSS 231.6 MB â†’ 249.7 MB (page 150) â†’ 246.4 MB (page 180) on the first run; 231.6 MB â†’ 241.2 MB â†’ 241.3 MB on the second. Roughly flat, not growing linearly with pages read |
...
Exact source (`docs/PHASE_1_PLAN.md` Â§7, "Compatibility"):
...
disabled just to boot) â€” never on the physical device running the identical sequence, and
...
version, which calls a static `InputManager.getInstance()` accessor this platform removed â€”
...
### TalkBack: manual checklist â€” PASS (owner-verified, 2026-09-24)
...
The owner ran the 7-item checklist in `PHASE1_CLAUDE_HANDOFF.md` â†’ "TalkBack manual
...
**TALKBACK: VERIFIED (PASS)**, owner-reported, genuine spoken/gesture evidence â€” not
...
| Check | Result |
| --- | --- |
| Codex R1â€“R5 regressions | 3/3 pass |
...
Every item from `PHASE_1_PLAN.md` Â§7 has now been exercised, with evidence recorded rather
than assumed, across three closure passes plus the original Phase 1 validation and Codex's
R1â€“R5 review and remediation verification:
...
- **Private acceptance, reliability:** the original four private samples, plus non-seekable
  provider, low storage, interrupted copy and Room transaction rollback â€” all covered and
  passing.
...
gates, per their own classification above â€” neither is a ShelfOS defect, neither was
...
None of these, nor anything above, reopens R1â€“R5.
...
- Compact phone portrait: bottom navigation; scrollable complete category names;
  2â€“3 column grid; selecting a cover opens details; Back returns to Library.
...
| Phone Android tests | 5 passed on AOSP API 35, 1080Ã—1920 at 420dpi |
| Expanded Android tests | Same 5 passed on AOSP API 35, 1600Ã—1000 at 160dpi |
...
| Window adaptation | Observed bottom navigation â†’ rail + details â†’ bottom navigation when resizing the same running emulator |
